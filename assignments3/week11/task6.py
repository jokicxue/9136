...
class MagicMultiContainer(Container):
    ...
    # class method to read file
    @classmethod
    def read_magic_multi_container(cls, file_name, multi_containers_file, containers, multi_container):
        '''
        This function is to set containers list
        '''
        ...
        with open(multi_containers_file, newline='') as original_data:
            ...
            for row in reader:
                ...
                multi_container = MagicMultiContainer(clean_row[0], containers_list)
                multi_containers.append(multi_container)
        ...
        with open(file_name, newline='') as original_data:
            ...
            for row in reader:
                clean_row0 = [field.strip() for field in row]
                sub_container_name = clean_row0[1]


                match_container0 = None
                for cont in range(len(multi_containers)):
                    if multi_containers[cont].cont_name == sub_container_name:
                        match_container0 = multi_containers[cont].cont_name
                        break
                '''
                if match_container:
                    # create a new container with the same default value
                    ...
                    containers_list.append(new_container)
                '''
                magic_multi_container = MagicMultiContainer(clean_row0[0], multi_containers[cont].containers)
