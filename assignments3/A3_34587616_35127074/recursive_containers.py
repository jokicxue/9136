...
class Gamesystem:
    ...
    def create_new_items(self, loot_items):
        ...
        """
        new_container = None
        # check the type of the item and create the object
            ...
            while True:
                ...
                if choice == "1":
                    while True:
                        ...
                        if not found:
                            for container in self.containers:
                                # if find in the container list
                                if container.name == choose_item:
                                    # create a new container
                                    new_container = self.create_new_items(container)
                                    # add the container into container
                                    if pick_container.add_item(new_container):
                                        print(f'''Success! Item "{new_container.name}" stored in container "{pick_container.name}".''')
                                    else:
                                        print(f'''Failure! Item "{new_container.name}" NOT stored in container "{pick_container.name}".''')
                                    found = True
                                    break
                        # if not find the item tell user try again
                        if not found:
